...
namespace GameSaveSystem
{
	...
	{
		/// <summary>
		/// Calculates the sort value for a file, using the LastWriteTimeUtc as a base and index as an offset (in case two files have the same exact LastWriteTimeUtc value.)
		...
		/// <summary>
		/// Takes a file name (either FileName.Extension or FileName.Index.Extension) and returns the base form (FileName.Extension.)
		/// </summary>
		...
		/// <summary>
		/// Takes a file name (either FileName.Extension or FileName.Index.Extension) and returns an incremental file name using the supplied <paramref name="index"/> (FileName.Index.Extension.)
		/// </summary>
		/// <param name="fileName">The file name to convert.</param>
		...
		public static string GetIncrementalFileName(string fileName, int index)
		{
			...
			return AddFileExtension(parts[0] + '.' + index.ToString(), parts.Length == 2 ? parts[1] : parts[2]);
		}
		...
		public static string IncrementFileName(string fileName, int maximumIndex)
		{
			...
			if(index++ > maximumIndex)
				index = 1;
			return parts[0] + '.' + index.ToString() + '.' + parts[2];
		}

		/// <summary>
		/// Returns a file search pattern (FileName.*.Extension) to find incremental files.
		/// </summary>
		/// <param name="fileName">The file name to use for creating the pattern (either FileName.Extension or FileName.Index.Extension.)</param>
		...
		/// <summary>
		/// Creates a valid file name, ensuring that a dot is placed between <paramref name="fileName"/> and <paramref name="extension"/>.
		/// </summary>
		...
		/// <param name="rootPath">The root path to the save folder.</param>
		/// <param name="fileName">The file name to use (in FileName.Extension format.)</param>
		/// <param name="forceRevert">True to skip the first result (in cases where the system can't tell that the file is invalid, but the player can), false otherwise.</param>
		...
		public static string LoadGame(string rootPath, string fileName, bool forceRevert, Func<string, bool> loadCallback)
		{
			...
			FileInfo[] fileInfos = (forceRevert ? fileInfosEnumerable.Skip(1) : fileInfosEnumerable).ToArray();
			if(fileInfos.Length > 0)
			{
		...
		/// <summary>
		/// Enumerates all of the save files, returning the base name (FileName.Extension) as well as the most recent incremental version (FileName.Index.Extension.)
		/// </summary>
		/// <param name="rootPath">The root path to the save folder.</param>
		/// <param name="fileExtension">The save file extension to use.</param>
		/// <returns>A list of pairs containing the base name (FileName.Extension) as the Key and the incremental file (FileName.Index.Extension) as the Value.</returns>
